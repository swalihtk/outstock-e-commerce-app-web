...
function ListUsers() {
    ...
    function getActiveUsers(){
        setLoading(true);
        axios.get("/admin/users/active").then(response=>{
    ...
    function getBlockedUsers(){
        setLoading(true);
        axios.get("/admin/users/blocked").then(response=>{
                    ...
                    <Table className={classes.table} aria-label="simple table">
                        ...
                        <TableBody>
                        {
                            listUsers&&listUsers.map((user, index)=>{
                                return <Users key={user._id} blocked={blocked} getActiveUsers={getActiveUsers} getBlockedUsers={getBlockedUsers} setListBlocked={setListBlocked} setListActive={setListActive} user={user} index={index+1}/>
                            })
                        }
                        </TableBody>
                    </Table>
