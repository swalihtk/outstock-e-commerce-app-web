...
function EditProduct({editHandlerFalse, productId}) {
...
/********* Redux *****/
const productState=useSelector(state=>state.productUpdateAdmin);
let dispatch=useDispatch();
let productObj=useSelector(state=>state.productOneAdmin.product)



/*********** Form Handler State ********/
let [name, setName]=useState(productObj.name);
let [price, setPrice]=useState(productObj.price);
let [details, setDetails]=useState(productObj.details);
let [shortDescription, setShortDescription]=useState(productObj.shortDescription);
let [color, setColor]=useState(productObj.color);
let [brand, setBrand]=useState(productObj.brand);
let [quanity, setQuantity]=useState(productObj.quanity);
...
let [previewSource1, setPreviewSource1]=useState(productObj.productImages&&productObj.productImages[0].img);
let [previewSource2, setPreviewSource2]=useState(productObj.productImages&&productObj.productImages[1].img);
let [previewSource3, setPreviewSource3]=useState(productObj.productImages&&productObj.productImages[2].img);
let [allImageFiles, setAllImageFiles]=useState([]);
let [allImageIds, setAllImageIds]=useState([]);

/********** Handle Image *********/
function previewImageOne(e){
    setAllImageFiles(prev=> [...prev, e.target.files[0]])
    setAllImageIds(prev=> [...prev,productObj.productImages[0]._id])
...
function previewImageTwo(e){
    setAllImageFiles(prev=> [...prev, e.target.files[0]])
    setAllImageIds(prev=> [...prev,productObj.productImages[1]._id])
    setPreviewImage(e.target.files[0], setPreviewSource2)
}

function previewImageThree(e){
    setAllImageFiles(prev=> [...prev, e.target.files[0]])
    setAllImageIds(prev=> [...prev,productObj.productImages[2]._id])
...
function uploadProduct(e){
    ...
    dispatch(updateProductAdmin(formData, body, productId));
...
return (
    <div>
        ...
        <Form className="pb-4" onSubmit={uploadProduct} id="productAdd" encType="multipart/form-data">
            ...
            <Row className="mt-4">
                    <Col>
                        <Form.Group controlId="formFileMultiple" className="mb-3">
                            ...
                            <Form.Control type="file" onChange={previewImageOne} name="image"/>
                        </Form.Group>
                    </Col>
                    <Col>
                        <Form.Group controlId="formFileMultiple" className="mb-3">
                            <Form.Label>Image2</Form.Label>
                            <Form.Control type="file" onChange={previewImageTwo} name="image"/>
                        </Form.Group>
                    </Col>
                    <Col>
                        <Form.Group controlId="formFileMultiple" className="mb-3">
                            <Form.Label>Image3</Form.Label>
                            <Form.Control type="file" onChange={previewImageThree} name="image"/>
                        </Form.Group>
                    </Col>
            </Row>
            <Row className="mt-4">
            ...
            <Button className="mb-4 mt-3 product-create-btn" type="submit">Save Product</Button>
            <Button className="mb-4 mt-3 product-create-btn btn-danger" onClick={editHandlerFalse}>Cancell Edit</Button>
        </Form>
    </div>
    </div>
)
