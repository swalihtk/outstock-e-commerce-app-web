...
function EditProduct(props) {
  ...
  // mount for product details
  useEffect(()=>{
    productHelper.getEditProductDetails(id, setProductDetails);
  }, [])
  useEffect(()=>{
    ...
    setQuantity(productDetails.quantity);
    setDiscription(productDetails.discription);

    // image
    if(!productDetails.productImages) return;
    let images=productDetails.productImages;
    setPreviewSource1(images[0].img)
    setPreviewSource2(images[1].img)
    setPreviewSource3(images[2].img)

    // category
    setMainCatValue(productDetails.category);
  }, [productDetails])
  ...
  function editFormHanlder(e){
    ...
    productHelper.editProductDetails(id, body, setupdateLoading, setShowEditForm);
  }
      ...
      <div className="productCreate__main container">
        ...
        <div className="row" style={{marginTop:"-1rem"}}>
          ...
          <div className="col-6 col-6 product__form">
            {
              updateLoading?
              <button style={{background:"#f0f0f0", color:"black"}} type="submit"><Spinner animation="border" size="sm" style={{marginRight:"1rem"}} variant="success" />UPDATING</button>
              :
              <button style={{background:"blue"}} type="submit">UPDATE</button>
            }
          </div>
        </div>
        </form>
